...
const transporter = nodemailer.createTransport({
  ...
  // Refer to Nodemailer documentation for the specific settings
  ...
  try {
    ...
    app.post("/savecomment", async (req, res) => {
      try {
        ...
        if (!blog) {
          ...
          const savedComment = await commentsCollection.insertOne({
            blogId: blog._id,
            comment,
            name,
            email,
    ...
    app.post("/addblog", verifyIsAuthor, async (req, res) => {
      try {
        ...
        console.log("Received Blog Data:", blog); // Log the received blog data to debug
    ...
    app.put("/authorprofilesettings/:authorId", verifyIsAuthor, async (req, res) => {
      try {
        ...
        if (authorResult.modifiedCount > 0) {
          ...
          if (blogResult.modifiedCount > 0) {
            ...
            res.json({ message: "Author profile and related blogs updated successfully" });
          } else {
